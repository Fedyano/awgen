...
add_client() {
    ...
    local CLIENT_PRIVATEKEY="${CLIENT_DIR}/${CLIENT_NAME}.privatekey"
    local CLIENT_PUBLICKEY="${CLIENT_DIR}/${CLIENT_NAME}.publickey"
    ...
    local BASE_SUBNET
    BASE_SUBNET=$(grep -o '172\.[0-9]*\.[0-9]*' /etc/amnezia/amneziawg/"$config"/"$config".conf | head -1)

    local LAST_OCTET
    LAST_OCTET=$(grep -o "${BASE_SUBNET}\.[0-9]*" /etc/amnezia/amneziawg/"$config"/"$config".conf | awk -F. '{print $4}' | sort -n | tail -n 1)

    if [[ -z "${LAST_OCTET}" ]]; then
        LAST_OCTET=1
    fi

    local CLIENT_IP="${BASE_SUBNET}.$((LAST_OCTET + 1))"
...
EOC
    ...
    systemctl restart awg-quick@"$config"
...
stop_client() {
    ...
    CLIENT_PUBLICKEY=$(cat /etc/amnezia/amneziawg/"$config"/clients/"$CLIENT_NAME"/"${CLIENT_NAME}".publickey)
...
delete_client() {
    ...
    systemctl restart awg-quick@"$config" || true
...
show_clients() {
    ...
    for client_dir in /etc/amnezia/amneziawg/"$config"/clients/*; do
        if [[ -d "$client_dir" ]]; then
            ...
            local endpoint
            endpoint=$(awg show "$config" endpoints | grep "$(cat "$client_dir/$client_name.publickey")" | awk '{print $2}')
            local allowed_ips
            allowed_ips=$(awg show "$config" allowed-ips | grep "$(cat "$client_dir/$client_name.publickey")" | awk '{print $2}')
            local last_handshake
            last_handshake=$(awg show "$config" latest-handshakes | grep "$(cat "$client_dir/$client_name.publickey")" | awk '{print $2}')
            local transfer_rx
            transfer_rx=$(awg show "$config" transfer | grep "$(cat "$client_dir/$client_name.publickey")" | awk '{print $2}')
            local transfer_tx
            transfer_tx=$(awg show "$config" transfer | grep "$(cat "$client_dir/$client_name.publickey")" | awk '{print $3}')
            local formatted_rx
...
uninstall_server() {
    ...
    log "$config" "Removing awgen and awdecode utilities..."
    progress_bar 10 &
    sudo rm -f /usr/local/bin/awgen /usr/local/bin/awdecode /usr/local/bin/awgdecoder.jar
    wait

    log "$config" "Removing systemd service..."
    progress_bar 10 &
    sudo systemctl disable awgen > /dev/null 2>&1
    ...
    progress_bar 10 &
    (crontab -l -u root 2>/dev/null | grep -v "find /etc/amnezia/amneziawg/$config/logs/") | crontab -u root -
    (crontab -l -u root 2>/dev/null | grep -v "ls -t /etc/amnezia/amneziawg/$config/backups/") | crontab -u root -
    wait
...
progress_bar() {
    ...
    total_intervals=$(echo "$duration / $interval" | bc)
    local completed_intervals=0
    while ((completed_intervals <= total_intervals)); do
        local percent=$((completed_intervals * 100 / total_intervals))
        echo -ne "\r\e[32mProgress : ["
        ...
        echo -ne "] ${percent}%\e[0m"
        completed_intervals=$((completed_intervals + 1))
        sleep "$interval"
    done
    echo -e "\r\e[32mProgress : [==================================================] 100%\e[0m"
}
...
if [[ "$1" == "add" ]]; then
    if [[ -z "$2" || -z "$3" ]]; then
        echo "Usage: $0 add <config_name> <client_name>"
        exit 1
    fi
    add_client "$2" "$3"
    exit 0
...
sudo apt-get update
sudo apt-get install -y qrencode bc openjdk-17-jre-headless
log_deploy "Необходимые пакеты установлены."

# Установка awgdecoder.jar, если необходимо
if [[ ! -f /usr/local/bin/awgdecoder.jar ]]; then
    AWGDECODER_URL="https://github.com/rakodin/awgdecoder/releases/latest/download/awgdecoder-1.0-SNAPSHOT-run.jar"  # Замените на актуальный URL
    log_deploy "Скачивание awgdecoder.jar..."
    sudo wget -O /usr/local/bin/awgdecoder.jar "$AWGDECODER_URL"
